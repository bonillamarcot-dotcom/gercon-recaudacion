...
# --- LÓGICA TÉCNICA (FÓRMULAS EXACTAS SOLICITADAS) ---

# 1. GENERACIÓN DE DESECHOS (Lógica de cascada)
ton_res_dia = round(poblacion * 0.0005, 2)    # Población * 0,0005
ton_com_dia = round(ton_res_dia * 0.20, 2)    # 20% de la Residencial
ton_ind_dia = round(ton_res_dia * 0.05, 2)    # 5% de la Residencial

ton_total_dia = round(ton_res_dia + ton_com_dia + ton_ind_dia, 2)
ton_total_mes_26 = round(ton_total_dia * 26, 2)

# 2. CENSO FÍSICO (Distribución 60-30-10 sobre censo de 5161 viviendas base)
...
# 3. BASE DE RECAUDACIÓN (Unidades Financieras del Excel)
...
data_gen = {
    "Sector": ["Residencial (Pob * 0.0005)", "Comercial (20% Res)", "Industrial (5% Res)", "TOTAL"],
...
with col_res:
    st.markdown("### **Bloque Residencial (60-30-10)**")
    st.write(f"- Sector Popular: **{u_pop_censo:,} Uds**")
    st.write(f"- Sector Medio: **{u_med_censo:,} Uds**")
    st.write(f"- Sector Alto: **{u_alt_censo:,} Uds**")
with col_esp:
...
def generate_pdf():
    ...
    pdf.cell(0, 10, f"Generación Mensual (26 días): {ton_total_mes_26:.2f} Ton", ln=True)
    pdf.cell(0, 10, f"Recaudación Real Estimada: $ {total_real:,.2f}", ln=True)
